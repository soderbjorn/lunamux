#!/usr/bin/env bash
# Starts the Lunamux server on the debug TLS port (8444) so it can run
# alongside a production instance on the default TLS port (8443).
#
# The debug server is pointed at a separate SQLite database (lunamux-dev.db)
# via -Dlunamux.dbPath, so it never shares on-disk state with a prod server
...
# file (and its WAL). This is the same dev DB that scripts/delete-all-config.sh
