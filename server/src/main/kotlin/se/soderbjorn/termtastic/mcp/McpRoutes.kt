/**
 * Ktor transport for the Termtastic MCP server: the `/mcp` streamable-HTTP
 ...
 *
 * Mounted from [se.soderbjorn.termtastic.module] at the end of the
 * routing block.
 *
 * @see McpServer
...
import se.soderbjorn.termtastic.readAuthToken
import se.soderbjorn.termtastic.readClientInfo
...
private suspend fun ApplicationCall.mcpAuthorize(repo: SettingsRepository): McpScope? {
    ...
    if (!repo.isMcpEnabled()) {
        respond(HttpStatusCode.Forbidden, "MCP is disabled in Termtastic settings")
        return null
    }
