 ...
 * tail, e.g. `Pollinating… (17m 48s · ↑ 51.6k tokens)` — the `↑ … tokens)`
 * shape is Claude-specific and survives the truncation that hides the
 * primary marker. In even narrower panes the `· ↑ tokens)` tail is also
 * truncated, leaving only `Synthesizing… (4m 33s)`; the gerund + Unicode
 * ellipsis + paren-timer combination is matched as a third-tier signal.
 *
...
object StateDetector {
    ...
    // The "\u2191 <num>[k] tokens)" tail is Claude-specific and survives the
    // narrow-pane truncation that hides the "esc to interrupt" affordance.
