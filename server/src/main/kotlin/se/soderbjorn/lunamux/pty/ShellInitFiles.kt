...
import se.soderbjorn.lunamux.persistence.AppPaths
import java.io.File
 ...
 *    `${'$'}{LUNAMUX_REAL_ZDOTDIR:-${'$'}HOME}`), then in `.zshrc` restores
...
internal object ShellInitFiles {
     ...
     * `LUNAMUX_REAL_ZDOTDIR` by [configureZsh]) or `${'$'}HOME` when the user
     * had none. We deliberately do NOT restore `ZDOTDIR` or clear
     * `LUNAMUX_REAL_ZDOTDIR` here — later phases (`.zprofile`, `.zshrc`)
     ...
     * dir as `LUNAMUX_REAL_ZDOTDIR`, so `.zshrc` still runs and then
    ...
    private fun zshForwardContents(fileName: String): String = """
        # lunamux zsh bootstrap ($fileName) — forward to the user's real
        ...
        __lunamux_real_zdotdir="${'$'}{LUNAMUX_REAL_ZDOTDIR:-${'$'}HOME}"
        __lunamux_boot_zdotdir="${'$'}ZDOTDIR"
        [[ -r "${'$'}__lunamux_real_zdotdir/$fileName" ]] && source "${'$'}__lunamux_real_zdotdir/$fileName"
        ...
        if [[ "${'$'}ZDOTDIR" != "${'$'}__lunamux_boot_zdotdir" ]]; then
          export LUNAMUX_REAL_ZDOTDIR="${'$'}ZDOTDIR"
          export ZDOTDIR="${'$'}__lunamux_boot_zdotdir"
        fi
        unset __lunamux_real_zdotdir __lunamux_boot_zdotdir
    """.trimIndent() + "\n"
    ...
    private fun configureZsh(env: MutableMap<String, String>) {
        ...
        env["ZDOTDIR"]?.let { env["LUNAMUX_REAL_ZDOTDIR"] = it }
    ...
    private fun zshRcContents(): String = """
        # lunamux zsh bootstrap — sources the real .zshrc, then installs an
        # OSC 7 emitter so the host server can track this pane's cwd.
        #
        # We restore ZDOTDIR (and clear LUNAMUX_REAL_ZDOTDIR) BEFORE sourcing
        ...
        if [[ -n "${'$'}LUNAMUX_REAL_ZDOTDIR" ]]; then
          local __lunamux_real_zdotdir="${'$'}LUNAMUX_REAL_ZDOTDIR"
          export ZDOTDIR="${'$'}__lunamux_real_zdotdir"
          unset LUNAMUX_REAL_ZDOTDIR
          [[ -r "${'$'}__lunamux_real_zdotdir/.zshrc" ]] && source "${'$'}__lunamux_real_zdotdir/.zshrc"
          unset __lunamux_real_zdotdir
        else
          unset ZDOTDIR
        ...
        chpwd_functions+=(__lunamux_emit_osc7)
