/**
 * Self-signed TLS keystore lifecycle for the Lunamux server.
 *
 ...
 *
 * Called by [se.soderbjorn.lunamux.main] immediately before the Ktor
 * `embeddedServer` is built; the returned bundle is fed into `sslConnector`.
 *
 * @see AppPaths.tlsDir
 * @see se.soderbjorn.lunamux.auth.DeviceAuth
 */
package se.soderbjorn.lunamux.tls
 ...
 *
 * Called once per server boot by [se.soderbjorn.lunamux.main]; the result
