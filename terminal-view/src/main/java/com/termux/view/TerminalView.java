...
public final class TerminalView extends View {
    ...
    /** Listener invoked whenever the pixel-measured terminal grid size changes. */
    public interface OnTerminalGridSizeChangedListener {
        void onTerminalGridSizeChanged(int cols, int rows);
    }
    ...
    public void setOnTerminalGridSizeChangedListener(OnTerminalGridSizeChangedListener listener) {
        ...
        if (listener != null && mLastGridCols > 0 && mLastGridRows > 0) {
            listener.onTerminalGridSizeChanged(mLastGridCols, mLastGridRows);
        }
    }
    ...
    public void setTextSize(int textSize) {
        mRenderer = new TerminalRenderer(
            ...
            mRenderer == null ? null : mRenderer.mFallbackTypeface);
        updateSize();
    }

    public void setTypeface(Typeface newTypeface) {
        mRenderer = new TerminalRenderer(mRenderer.mTextSize, newTypeface, mRenderer.mFallbackTypeface);
        updateSize();
        invalidate();
    }

    public void setFallbackTypeface(Typeface fallbackTypeface) {
        mRenderer = new TerminalRenderer(mRenderer.mTextSize, mRenderer.mTypeface, fallbackTypeface);
        updateSize();
        invalidate();
    }
    ...
    public void updateSize() {
        ...
        if (newColumns != mLastGridCols || newRows != mLastGridRows) {
            mLastGridCols = newColumns;
            mLastGridRows = newRows;
            if (mGridSizeListener != null) {
                mGridSizeListener.onTerminalGridSizeChanged(newColumns, newRows);
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        if (mEmulator == null) {
            ...
            // Guard against background emulator mutations (append/resize)
            // racing the renderer — resize changes mScreenRows mid-iteration
            // and TerminalBuffer throws, and append can invalidate row
